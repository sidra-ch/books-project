...
const Hero = ({ searchQuery }) => {
      ...
      try {
        ...
        const response = await fetch(
          `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(
            searchQuery
          )}&maxResults=35&key=AIzaSyBTxHEqiN2DbRx3x2-y43MEbXXiBhwSbyg`
        );
  ...
  if (loading) {
    return (
      <div className="animate-purple grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 p-6">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-6 md:mt-8 flex flex-wrap justify-center items-center gap-2 md:gap-3">
          <button
            ...
            className="px-3 py-1 md:py-2 rounded-md md:rounded-lg bg-gray-900/80 border border-gray-700/50 text-cyan-300 text-xs md:text-sm hover:bg-gray-900/60 transition-colors disabled-opacity-50"
          >
            Previous
          </button>
          {generatePageNumber().map((page, index) => (
            <button
              ...
              className={`px-3 py-1 md:py-2 rounded-md md:rounded-lg text-xs md:text-sm ${
                page === currentPage
